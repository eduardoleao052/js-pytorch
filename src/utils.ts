...
export function getShape(data: NestedArray<number> | number, shape: Array<number> = []): Array<number> {
  // Base case:
  ...
  // Iterate over elements in dimention to check if all have the same length (homogenous shape):
    for (const element of data) {
      ...
      if (Array.isArray(element) && elementLength != element.length ) {
        throw new Error("The requested array has an inhomogeneous shape.");
 ...
 */
export function getData(a: Tensor | Array<number> | number): Array<number> | number {
  if (Array.isArray(a)) {
    return a;
  }
  if (typeof a === "number") {
